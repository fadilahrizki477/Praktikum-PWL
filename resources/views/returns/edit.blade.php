<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Pengembalian</h2></x-slot>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
            <form method="POST" action="{{ route('return.update', $return->id) }}">
                ...
                <div class="mb-4">
                    ...
                    <select name="loan_detail_id" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm" required>
                        @foreach ($loanDetails as $detail)
                            <option value="{{ $detail->id }}" {{ old('loan_detail_id', $return->loan_detail_id) == $detail->id ? 'selected' : '' }}>
                ...
                <div class="mb-4 flex items-center gap-2">
                    <input type="checkbox" name="charge" id="charge" value="1" {{ old('charge', $return->charge) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600">
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="amount" value="{{ old('amount', $return->amount) }}" min="0" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm" required>
                </div>
